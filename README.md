**Author's README**
In this project I have aimed to learn usage of **React's** props and state along with Stateless Functional Components and Class Components
**_______________**
...
* Files with `.asddd.js` suffix.
* Files with `.spec.js` suffix.

The `.asddd.js` / `.spec.js` files (or the `__tests__` folders) can be located at any depth under the `src` top level folder.

We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `App.asddd.js` and `App.js` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
This test mounts a component and makes sure that it didn’t throw during rendering. Tests like this provide a lot of value with very little effort so they are great as a starting point, and this is the test you will find in `src/App.asddd.js`.
